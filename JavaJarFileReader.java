...
//import jarfile.TestingJars;

import java.nio.file.DirectoryNotEmptyException;

/**
 * Recursively find all ".java" and ".jar" files in a given directory. This
 * reader will extract all the JAR files, and retrieve source code from all
 * found JAVA files.
 * 
...
public class JavaJarFileReader {
	public static void main(String[] args) throws IOException {
		//test.extract("myFiles.jar","penis" );
		//test.extract("C:\\Users\\patri\\eclipse-workspace\\jarfile\\penismy", "penis\\fuck\\15\\18\\23");
		ArrayList<String> test1 = JavaJarFileReader.getAllFilesToString("C:\\Users\\patri\\eclipse-workspace\\jarFile");
		//System.out.println(Arrays.deepToString(test1.toArray()));
	...
	/**
	 * enter jar file and look for .java files return all contents in the form of a
	 * string
	 * 
	 * @param jarFileName
	 * @return String containing the contents of all .java files in the jar
	 * @thows IOException
	 * 
	 */
	public ArrayList<String> getJarJavaFiles(String jarFileName, String destP) throws IOException {
		ArrayList<String> totalJavaContents = new ArrayList<String>();

		JarFile jarFile = new JarFile(jarFileName);
		// get enumeration of all the file names in the jar file
		Enumeration<JarEntry> enumOfJar = jarFile.entries();
		
		
		// iterate through files in jar returning the contents of the files ending in
		// .java in the form of a string
		if (enumOfJar.hasMoreElements()) {
			totalJavaContents = recurseJarFile(enumOfJar, jarFile, totalJavaContents);
			jarFile.close();

			return totalJavaContents;
		} else {
			jarFile.close();
			return null;
		}
	 ...
	 * @param enumOfJar
	 * @param jarFile
	 * @param totalJavaContents
	 * @return string of .java files
	 * @throws IOException
	 */
	public ArrayList<String> recurseJarFile(Enumeration<JarEntry> enumOfJar, JarFile jarFile, ArrayList<String> totalJavaContents) throws IOException {

		String jarFileName = ((ZipEntry) enumOfJar.nextElement()).getName();

		if (enumOfJar.hasMoreElements() && isJavaFile(jarFileName)) {
			...
			totalJavaContents.add(out.toString());
			
			br.close();
			
			totalJavaContents = recurseJarFile(enumOfJar, jarFile, totalJavaContents);
			
		} else if (enumOfJar.hasMoreElements()) {
	...
	/**
	 * Given a JAR file, and the target directory path, extract the JAR file into
	 * the target path.
	 * 
	 * @param jarFile
	 *            The absolute path of the JAR file to be extracted
	 * @param destDir
	 *            Target path to extract the JAR file into
	 * @throws IOException
	...
	/**
	 * Recursively find all the JAR files from the given directory path, and store
	 * the path of the JAR file into a list.
	 * 
	 * @param dirPath
	...
	/**
	 * Recursive function to find all the JAR files located in the current directory
	 * and its sub directories.
	 * 
	 * @param currDirectory
	 *            Current directory path
	 * @return ArrayList<String> An ArrayList of type String containing all the
	 *         paths to all found JAR files located in the current directory and all
	...
	/**
	 * Read the content of a JAVA file, and returns the content of the file as a
	 * String.
	 * 
	 * @param file
	 *            Path of the JAVA file to be read
	...
	/**
	 * Recursively find all the JAVA files from the given directory path, and store
	 * the path of the JAVA file into a list.
	 * 
	 * @param dirPath
	 *            The directory of interest
	 * @return ArrayList<String> An ArrayList of type String containing all the
	 *         paths to all found JAVA files.
	 */
	...
	/**
	 * Recursive function to find all the JAVA files located in the current
	 * directory and its sub directories.
	 * 
	 * @param currDirectory
	 *            Current directory path
	 * @return ArrayList<String> An ArrayList of type String containing all the
	 *         paths to all found JAVA files located in the current directory and
	...
	// String destDir =
	// "/home/slchan/eclipse-workspace/SENG300G2/src/test/javaFileReaderTestPackage/";
